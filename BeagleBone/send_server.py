...
import serial          # IM920sとのシリアル通信用ライブラリ
import requests        # みらいサーバーへのHTTP送信用ライブラリ
...
SERVER_URL = "https://mirai-server.example.com/api/sensor"  # みらいサーバーの送信先URL(要変更)
SERVER_TIMEOUT = 5                  # サーバーへの送信タイムアウト(秒)
...
def send_to_server(parsed_data, raw_line):
    """解析済みデータをみらいサーバーへHTTP POSTで送信する関数"""
    payload = {                                              # サーバーに送るJSONデータを作成
        "gateway_id": GATEWAY_ID,                            # ゲートウェイID
        "node_id": parsed_data["node_id"],                   # センサノード番号
        "rssi_hex": parsed_data["rssi_hex"],                 # 受信電波強度(16進数のまま)
        "payload_hex": parsed_data["payload_hex"],           # センサデータ(16進数のまま)
        "raw_line": raw_line,                                # 元の生データ行(念のため保持)
        "timestamp": time.time(),                            # 送信時刻(UNIXタイムスタンプ)
    }
    try:
        response = requests.post(                            # サーバーへPOSTリクエスト送信
            SERVER_URL,                                       # 送信先URL
            json=payload,                                     # JSON形式で送信
            timeout=SERVER_TIMEOUT                            # タイムアウト設定
        )
        if response.status_code == 200:                       # 成功(200)が返ってきたら
            logging.info(f"送信成功: node={parsed_data['node_id']}")  # 成功ログを記録
        else:                                                   # 200以外が返ってきたら
            logging.warning(f"送信失敗(status={response.status_code}): {response.text}")  # 警告ログ
    except requests.exceptions.RequestException as e:          # 通信エラーが発生した場合
        logging.error(f"サーバー送信エラー: {e}")               # エラー内容をログに記録
